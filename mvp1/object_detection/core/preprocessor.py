    ...
    with tf.name_scope("RandomDistortColor", values=[image]):
        if color_ordering == 0:
            image = random_adjust_brightness(
                image, max_delta=32.0 / 255.0, preprocess_vars_cache=preprocess_vars_cache
            )
        ...
        if pad_to_max_dimension:
            ...
            new_image = tf.stack(
                [
                    tf.pad(
                        channels[i],
                        [[0, max_dimension - new_size[0]], [0, max_dimension - new_size[1]]],
...
def resize_to_min_dimension(image, masks=None, min_dimension=600):
  ...
  """
    ...
    with tf.name_scope("ResizeGivenMinDimension", values=[image, min_dimension]):
        ...
        target_ratio = tf.to_float(min_target_dimension) / tf.to_float(min_image_dimension)
        ...
        image = tf.image.resize_bilinear(
            tf.expand_dims(image, axis=0), size=[target_height, target_width], align_corners=True
        )
...
def ssd_random_crop_pad(
  ...
  """
    ...
    return _apply_with_random_selector_tuples(
        tuple(
            t
            for t in (image, boxes, labels, label_weights, label_confidences, multiclass_scores)
            if t is not None
        ),
...
def get_default_func_arg_map(
  ...
  """
    ...
    prep_func_arg_map = {
        ...
        random_pad_image: (fields.InputDataFields.image, fields.InputDataFields.groundtruth_boxes),
