...
class BalancedPositiveNegativeSampler(minibatch_sampler.MinibatchSampler):
    ...
    """
        if positive_fraction < 0 or positive_fraction > 1:
            raise ValueError(
                "positive_fraction should be in range [0,1]. " "Received: %s." % positive_fraction
            )
    ...
    """
        ...
        return tf.cast(
            tf.tensordot(tf.cast(sampled_idx_indicator, tf.float32), reprojections, axes=[0, 0]),
            tf.bool,
        )
    ...
    """
        ...
        if len(labels.get_shape().as_list()) != 1:
            raise ValueError(
                "labels must be 1 dimensional, got a tensor of " "shape %s" % labels.get_shape()
            )
